...
(defn take-rand!
  ...
  [^long n nums]
  (let [{:keys [buf ^long len nexti$]} nums ; nums is structure with Neandertal vec of rand nums and an index
        ^long nexti @nexti$
    ...
    (let [new-nexti (if (<= n dist-to-end)
                         ...
                         (do (shift-and-refill! (:rng nums) buf nexti dist-to-end) ; need to stuff more numbers after the unused ones
                             ;(println "take-rand! called shift-and-refill! with nexti" nexti) ; DEBUG
...
(comment
  ...
  (def mynums (make-nums rng 10))
  (into [] (take-rand! 3 mynums))
  (into [] (:buf mynums))
)
