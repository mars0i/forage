...
(defn walk-experiments
  ...
  the params map.  walk-fns should be a map with keys that are strings, and
  values that are functions of one argument, an initial location from which
  to start generated by the function. The keys in walk-fns should be
  strings that can be used as names of the corresponding functions for
  reporting purposes, and should be suitable for embedding in filenames.
  Then for each walk-fn runs walks-per-combo food searches using the
  walk-fn and look-fn for each direction in init-dirs. A new PRNG is
  created using seed unless a PRNG is passed in for parameter rng, in which
  case the seed is just used as an id number. Output filenames (see below)
  are constructed using seed, values of :basename and :dirname, and in some 
  cases a walk-fns key. Output:
