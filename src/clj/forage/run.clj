...
(defn walk-experiments
  ...
  reporting purposes. Then for each walk-fn runs walks-per-combo food
  searches using the walk-fn and look-fn for each direction in init-dirs.
  A new PRNG is created using seed unless a PRNG is passed in for parameter
  rng, in which case the seed is just used as an id number. Output
  filenames (see below) are constructed using file-prefix and seed.
  Output:
