...
(defn live-composite-vecs
  ...
  sequence of functions like those generated by step-vector-fn.
  switch-fns is a sequence of functions, each of which can determine,
  on each step, whether to shift cyclically from one step vector
  function to the next to generate the next vector.  Each switch
  function should return an element that's either: (A) a truthy
  element which both means \"continue with this step vector\"
  function, and contains data to be passed to the same switch function
  on the next iteration in order to help it decide then, or (B) nil or
  false, indicating that the next step vector function should be used
  to generate the next step.  Each switch function should accept two
  arguments: (1) the new step vec, and (2) data from the last
  application of the same switch function (or nil if the previous
  application used a different switch function).  [Example data as
  second arg of switch functions: (i) number of steps in subsequence,
  (ii) length of subsequence, or (iii) the entire subsequence.]
  labels argument is experimental.  Could be used to distinguish
  subwalks, but only if endpoints are duplicated."
  ([switch-fns vec-fns]
   (live-composite-vecs switch-fns vec-fns nil))
...
(defn switch-after-n-steps-fn
  "Generates a dist?fn for use with live-composite-vecs.  The returned function
...
(comment
  ;; BASIC TESTS OF live-composite-vecs
  (def seed (r/make-seed))
  ...
  (def cb-vecs (live-composite-vecs [samedistfn] [vecfn]))
  (= (take 1000 levy-vecs) (take 1000 cb-vecs))
)
(comment
  ;; ADDITIONAL TESTS OF live-composite-vecs
  ...
  (def vecs (live-composite-vecs [switch10 switch10000] [vecfn1 vecfn3]))
  ;(take 200 vecs)
  ...
  ;(def vecs (live-composite-vecs [switch1] [vecfn1 vecfn3] ["mu=1" "mu=3"]))
  ;(def v0 (take 20 vecs))
