...
(defn make-composite-vecs
  ...
  dist?-fns is a sequence of functions that determine on each step
  whether to shift, cyclically, from one step vector function to the
  next in order to generate the next vector.  Each dist? function should
  return an element that's either: (A) a truthy element which both means
