...
;; algorithm:
;; generate next step
;; conj it onto the step seq
;; past new step along with carry-over data to dist? fn
;; if that wants same dist, then recurse with
;;    same fn cycles, and
;;    new data returned by dist?, and
;;    current step list
;; else recurse with
;;    shifted fn cycles (to start a new subseq), and
;;    empty data for use by dist?, and
;;    current step list
(defn composite-brownian-vecs
  "Returns an infinite sequence of mathematical vectors.  step-vec-fns
  is a sequence of functions like those generated by step-vector-fn.
  dist?-fns is a sequence of functions that determine whether to shift,
  cyclicly, from one step vector function to the next in order to
  generate the next vector.  Each dist? function should return an
  element that's either: (A) a truthy element which both means
  \"continue with this step vector\" function, and contains data to be
  passed to the same dist? function on the next iteration in order to
  help it decide then.  (B) nil or false, indicating that the next step
  vector function should be used to generate the next step.  Each dist?
  function should accept two arguments: (1) the new step vec, and (2)
  data from the last application of the same dist? function (or nil if
  the previous application used a different dist? function).  [Example
  data as second arg of dist?  functions: (i) number of steps in
  ...
  (loop [vecfns (cycle vec-fns), dist?fns (cycle dist?-fns), dist?data nil, vecs []]
    (let [vecfn (first vecfns)
          ...
          new-vec (vecfn)
          new-vecs (conj vecs new-vec)]
      (if-let [newdist?data (dist?fn new-vec dist?data)] ; truthy means "keep using this vec fn"
        (recur vecfns dist?fns newdist?data new-vecs)
        (recur (rest vecfns) (rest dist?fns) nil new-vecs))))) ; nil since new vecfn, dist?fn should restart counting, etc.
...
(defn levy-foodwalk
  ...
  ([look-fn look-eps maxpathlen init-dir trunclen dir-dist len-dist init-pad init-loc]
   (let [raw-inf-step-walk (levy-vecs dir-dist len-dist 1 trunclen)
