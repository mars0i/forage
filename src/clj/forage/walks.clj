...
(defn make-composite-vecs
  ...
  sequence of functions like those generated by step-vector-fn. dist?-fns
  ...
  generate the next vector.  Each dist? function should return an element
  that's either: (A) a truthy element which both means \"continue with this
  step vector\" function, and contains data to be passed to the same dist?
  function on the next iteration in order to help it decide then, or (B) nil
  or false, indicating that the next step vector function should be used to
  generate the next step.  Each dist? function should accept two arguments:
  (1) the new step vec, and (2) data from the last application of the same
  dist? function (or nil if the previous application used a different dist?
...
(comment
  ...
  ;(def vecfn1 (step-vector-fn rng1 lendist1 1 100))
  ...
  (def switch100 (switch-after-n-steps-fn 100))

  (def vecs (make-composite-vecs [vecfn2 vecfn3] [switch100]))
  (def walk (vecs-upto-len 2000 vecs))
