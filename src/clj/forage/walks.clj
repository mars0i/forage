...
(defn composite-brownian-vecs
  ...
  tran-fns is a sequence of functions that determine whether to shift,
  cyclicly, from one step vector function to the next in order to
  generate the next vector.  Each tran function should return an element
  that's either: (A) a truthy element which both means \"continue with
  this step vector\" function, and contains data to be passed to the
  same tran function on the next iteration in order to help it decide
  then.  (B) nil or false, indicating that the next step vector function
  should be used to generate the next step.  Therefore, each tran
  function should accept an argument which is the data from the last
  application of the same tran function or nil if the previous
  application used a different tran function.  [Example data for tran
  functions: (i) number of steps in subsequence, (ii) length of
  subsequence, or (iii) the entire subsequence.]"
  [step-vec-fns tran-fns]
  (if (not= (count step-vec-fns) (count tran-fns)) ; quick sanity check (TODO replace with spec)
    (println "composite-brownian-vecs failed:" (count step-vec-fns) "step-vec-fns and" (count tran-fns) "tran-fns")
    (let [step-vec-cycle (cycle step-vec-fns)
          tran-cycle (cycle tran-fns)]
      (loop [next-step-fns step-vec-cycle
             next-tran-fns tran-cycle
             tran-data nil
             vecs []]
              ;; TODO somewhere test whether to switch tran fns and step fns
