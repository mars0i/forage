  ...
  * Returns map with keys:
  ...
  ([params walk-fns walks-per-fn seed rng]
   (let [num-dirs (params :num-dirs)
         ...
         dont-print (params :dont-print)
         dont-save (params :dont-save)
         base-filename (if dont-save nil (str (params :dirname) (params :basename) seed))
         param-filename (if dont-save nil (str base-filename "params.csv"))
         data-filename(if dont-save nil (str base-filename "data.csv"))
         base-state-filename (if dont-save nil (str base-filename "state") ); for PRNG state files
         sorted-params (if dont-save nil (into (sorted-map) params) ); for writing param file
         param-labels (if dont-save nil (append-labels (concat ["namespace" "seed"] (keys sorted-params))))
         param-data (if dont-save nil (append-row param-labels
                                (cons (str *ns*)
                                      (cons (str "\"" seed "\"") ; keep Excel from making it a float
                                            (vals sorted-params)))))
         runids (range 1 (inc walks-per-fn))
         path-labels (if dont-save nil (map #(str "path " %) runids))   ; labels for path lengths until found or gave up
         data$ (if dont-save (atom nil) (atom (append-labels (into ["initial dir" "walk-fn" "segments"
     ...
     (when-not dont-save (csv/spit-csv param-filename param-data)) ; write out fixed parameters
     (when-not dont-print
     ...
     (when-not dont-save (csv/spit-csv data-filename @data$))
     (when-not dont-print (println " done."))
