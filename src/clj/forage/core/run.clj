...
(defn walk-experiments
  ...
  if it was passed as an argument). Seed is only used for informational
  purposes in file output, but should be the seed for the PRNG that is used by
  ...
  for each direction in init-dirs.  seed is used only as an id number, e.g. in
  ...
  filenames (see below) are constructed using seed, values of :basename and
