...
(defn run-and-collect
  ...
  It's usually defined in the relevant env namespace, or can be replaced with
  the identity function if coordinates will be ignored."
...
(defn walk-experiments
  "Runs several foraging simulations based on inputs, writes output data and
  possibly PRNG states to files, and returns data (and the PRNG that was used
  if it was passed as an argument). runs-label is only used for informational
  purposes in file output, but should be the runs-label for the PRNG that is used by
  the simulations. Uses parameters in the params map.  walk-fns should be a map
  with keys that are strings, and values that are functions of one argument, an
  initial location from which to start generated by the function.  Each such
  walk-fn should return a triple in which the first element is a collection of
  one or more found foodspots, or nil if none were round, a path until the
  foodspot(s) were found, and the rest of the path that would have been taken
  if no foodspot(s) were found.  walk-fn values should have the same PRNG
  embedded in them; that PRNG can be passed to this function. Keys in walk-fns
  should be strings or other values that can be used as names of the
  corresponding functions for reporting purposes, and should be suitable for
  embedding in filenames. (The str function will be used to convert the keys to
  strings if they are not already strings.) Then for each walk-fn, we run
  walks-per-combo food searches using the walk-fn (with look-fn embedded in it)
  for each direction in init-dirs.  runs-label is used only as an id number, e.g. in
  filenames.  The PRNG rng is never used, but its state is stored each time
  through the main loop, prior to each call to run-and-collect.  Output
  filenames (see below) are constructed using runs-label, values of :basename and
  :dirname, and in some cases a walk-fns key. Output:
   ...
   (let [num-dirs (params :num-dirs)
     ...
     (doseq [[walk-name env-name :as walk-key] (keys walk-fns)  ; doseq and swap! rather than for to avoid lazy chunking of PRNG
       ...
       (let [walk-fn (walk-fns walk-key) ; remaining arg is initial location [walk-name is a string, so can't be first]
             ...
             ;; old stats:
             ;n-found (count (keep identity found)) ; OLD VERSION (delete this)
             n-found (apply + found-counts)
